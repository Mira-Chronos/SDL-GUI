...
public:
	...
	virtual void on_press() {
		call_callbacks(WidgetEvent::ON_PRESS);
	}
	virtual void on_release() {
		call_callbacks(WidgetEvent::ON_RELEASE);
	}
	virtual void on_key_press(SDL_Scancode key) {
		call_callbacks(WidgetEvent::ON_KEY_PRESS);
	}
	virtual void on_key_release(SDL_Scancode key) {
		call_callbacks(WidgetEvent::ON_KEY_RELEASE);
	}
	virtual void on_hover();
	virtual void off_hover();
	virtual void on_select() {
		call_callbacks(WidgetEvent::ON_SELECT);
	}
	virtual void on_deselect() {
		call_callbacks(WidgetEvent::ON_DESELECT);
	}
	virtual void on_text_input(char* text) {
		call_callbacks(WidgetEvent::ON_TEXT_INPUT);   // this doesnt pass the argument!
	}
	virtual void on_wheel_up() {
		call_callbacks(WidgetEvent::ON_WHEEL_UP);
	}
	virtual void on_wheel_down() {
		call_callbacks(WidgetEvent::ON_WHEEL_DOWN);
	}
	...
	inline void set_tooltip_delay(float seconds) {
	...
	inline unsigned int get_column() {
		return column;
	}
	inline void set_row(unsigned int new_row) {
		row = new_row;
	}
	inline void set_column(unsigned int new_column) {
		column = new_column;
	}
protected:
